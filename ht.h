...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), size_(0),resizeAlpha_(resizeAlpha), mIndex_(0), num_deleted(0)
{
...
{
  if ((size_+num_deleted)/CAPACITIES[mIndex_]>=resizeAlpha_){
    resize();
  }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
  ...
  for (size_t i =0; i< oldtable.size(); i++){
    if (oldtable[i]!=nullptr){
      ...
      HASH_INDEX_T index=probe(oldtable[i]->item.first);
      if (index==npos){
        throw std::logic_error("nonexistent");
      }
      table_[index]=oldtable[i];
    }
  }
  //complete this
